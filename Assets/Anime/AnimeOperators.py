...
class Anime_SnapRender(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for index, frame in enumerate(frames):
            ...
            print("Render Finish: " + str(frame) + "frames | " +"%04d" % order_number + "/" + str(len(frames)))
            print("Rendering",
                  f'|{"█" * ((order_number) * 50 // len(frames)):50}|', 
                  f'{round((order_number * 100 // len(frames)))}% Progressing...', end = '\r')
