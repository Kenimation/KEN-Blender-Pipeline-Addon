    ...
    if obj.RIG_ID == "Horse - Anime":
        ...
        if any(item.registered_name == AnimeProperties.registered_name[2] for item in addon_prefs.registered_name):
            ...
            if rig.horse_rig_class == "Setup":
                ...
                box = layout.box()
                row = box.row()
                row.prop(rig, "horse_sub", toggle = True)
                row.prop(rig, "horse_lineart", toggle = True)
                row = box.row()
...
bpy.types.Object.horse_sub = BoolProperty(
    default=False, name = "Subdivision")

bpy.types.Object.horse_lineart = BoolProperty(
    default=False, name = "LineArt")
