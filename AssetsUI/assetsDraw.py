...
from . import assetsProperties
from .. import minecraftUI
from .. import animeUI
from ..Anime import AnimeProperties, AnimeExtraProperties
...
def draw_properties(addon_prefs, context, row, obj, pcoll):
    ...
    ken_icon02 = pcoll["Kenimation_02"]
    scene = context.scene
    if obj.type == 'ARMATURE':
        ...
        if scene.ken_rig == True:
            ...
            rig_icon = ken_icon
        if obj.mode != 'EDIT' and context.active_object.get("RIG_ID") in AnimeProperties.kenriglist:
...
def draw_ken_mcrig(addon_prefs, self, context, obj):
    scene = context.scene
    if obj.get("RIG_ID") == "KENRemastered":
        if obj.mode == 'OBJECT':
            if obj.type == 'ARMATURE':
                rig_class = "myrig"
        ...
        if scene.ken_rig == True:
            ...
            if rig.mode == 'EDIT':
                ...
                if rig_get_class == "one":
                    minecraftUI.drawrigdesign(self, context)
                if rig_get_class == "two":
                    minecraftUI.drawrigmaterial(self, context)
                if rig_get_class == "three":
                    minecraftUI.drawrigposing(self, context)
                if rig_get_class == "four":
                    minecraftUI.drawriganimating(self, context)
            if scene.myrig_ui_setting == True:
                minecraftUI.drawriguisetting(self, context)

    if obj.get("RIG_ID") == "KENLayout":
        if obj.mode == 'OBJECT':
            if obj.type == 'ARMATURE':
                rig_class = "riglayout"
        ...
        if scene.ken_rig == True:
            ...
            row.prop(scene, rig_class, expand = True)
            if rig_get_class == "one":
                minecraftUI.drawlayoutsetting(self, context)
            if rig_get_class == "two":
                minecraftUI.drawlayoutposing(self, context)
            if rig_get_class == "three":
                minecraftUI.drawriganimating(self, context)

def draw_ken_animerig(addon_prefs, self, context, obj):
    scene = context.scene
    rig = context.active_object
    if obj.get("RIG_ID") == "KenAnime":
        if obj.mode == 'OBJECT':
            if obj.type == 'ARMATURE':
        ...
        if scene.ken_rig == True:
            ...
            box = layout.box()
            row = box.row()
            row.label(text = "KEN Anime Rig v1.0", icon = "OUTLINER_OB_ARMATURE")
            row.prop(scene, "object_properties", icon = "ARMATURE_DATA", text = "")
            if rig.mode == 'EDIT':
                drawbone_properties(box, context, obj)
            else:
                ...
                row.prop(scene, rig_class, expand = True)
                if rig_get_class == "one":
                    animeUI.drawrigdesign(self, context)
                if rig_get_class == "two":
                    animeUI.drawrigmaterial(self, context)
                if rig_get_class == "three":
                    if rig_class != "myanimerigAddition":
                        animeUI.drawrigposing(self, context)
                    else:
...
def drawlight(box, light):
    ...
    box.prop(light, "color", text = "Color")
