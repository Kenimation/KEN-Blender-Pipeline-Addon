...
class Constraints_OT_Delete_Constraints(Operator):
	...
	def execute(self, context):
		...
		if context.space_data.context == 'CONSTRAINT':
			constraints = obj.constraints[obj.con_index]
			owner = "OBJECT"

		elif context.space_data.context == 'BONE_CONSTRAINT':
			constraints = bone.constraints[obj.bcon_index]
			owner = "BONE"
...
class Constraints_OT_Toggle_Constraints_View(bpy.types.Operator):
	...
	def execute(self, context):
		...
		bone = context.active_pose_bone
		if context.space_data.context == 'CONSTRAINT':
			con = obj.constraints[obj.con_index]
			if con.enabled == True:
				con.enabled = False
			else:
				con.enabled = True
			...
			con = bone.constraints[obj.bcon_index]
			if con.enabled == True:
				con.enabled = False
			else:
				con.enabled = True
...
class Constraints_OT_Solo_Constraints_View(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if context.space_data.context == 'CONSTRAINT':
			for con in obj.constraints:
				if con != obj.constraints[obj.con_index]:
					con.enabled = False
				else:
					con.enabled = True
			...
			for con in bone.constraints:
				if con != bone.constraints[obj.bcon_index]:
					con.enabled = False
				else:
					con.enabled = True
