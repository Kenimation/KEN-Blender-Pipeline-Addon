...
class ModifiersList_OT_Delete_Modifier(Operator):
	...
	def execute(self, context):
		...
		if obj.type != "GPENCIL":
			modifier = obj.modifiers[obj.mod_index]
			obj.modifiers.remove(modifier)
		else:
			modifier = obj.grease_pencil_modifiers[obj.mod_index]
			obj.grease_pencil_modifiers.remove(modifier)
...
class ModifiersList_OT_Solo_Modifiers_View(bpy.types.Operator):
	...
	def execute(self, context):
		...
		for obj in context.selected_objects:
			for mod in obj.modifiers[:]:
				if mod != obj.modifiers[obj.mod_index]:
					mod.show_viewport = False
				else:
					mod.show_viewport = True
				
			for mod in obj.grease_pencil_modifiers[:]:
				if mod != obj.grease_pencil_modifiers[obj.mod_index]:
					mod.show_viewport = False
				else:
					mod.show_viewport = True
...
def drawmodifiers(self, context, layout, row, obj):
	...
	if context.scene.mod_panel == True:
		if obj.modifiers or obj.grease_pencil_modifiers:
			try:
				if obj.type != "GPENCIL":
					modifiers = obj.modifiers[obj.mod_index]
				else:
					modifiers = obj.grease_pencil_modifiers[obj.mod_index]
