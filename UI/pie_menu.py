...
from ..Assets.Anime import AnimeProperties
...
class VIEW3D_MT_PIE_OBJECT(Menu):
    ...
    def draw(self, context):
        ...
        if not ob or not ob.select_get():
            ...
            pie_linking = pie.box().split().column()
            ...
            if context.scene.render.engine == 'CYCLES':
                box.prop(light, "use_shadow", text = "Shadow")
...
class VIEW3D_OT_Open_Object_PieMenu(Operator):
    ...
    def execute(self, context):
        ...
        if addon_prefs.pie_menu:
            if any(item.registered_name == AnimeProperties.registered_name[0] for item in addon_prefs.registered_name):
