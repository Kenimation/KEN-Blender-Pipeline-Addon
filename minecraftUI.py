...
from .Minecraft import minecraftExtraProperties
from .Minecraft import minecraftProperties
from .Minecraft import minecraftOperators
...
def drawlayoutsetting(self, context):
    ...
    if rig.solid == '0':
        ...
        row = box.row(align = True)  
        col = row.column()
        col.operator("packimg.thomasminecraftrig" , text="", icon="UGLYPACKAGE" if minecraftDefs.is_packed(skin_texture) else "PACKAGE").id_name = skin_texture.name 

        col = row.column() 
        if minecraftDefs.is_packed(skin_texture) == False:
            col.enabled = False
        ...
        if minecraftDefs.is_packed(skin_texture) == False:
            col.enabled = False
        col.operator("reloadimg.thomasminecraftrig", text = "", icon = "FILE_REFRESH").id_name = skin_texture.name 
...
def drawrigdesign(self, context):
    ...
    row = col.row()
    row.prop(rig, "NoFace", expand = True)
...
class MC_RIG_Menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        pcoll = preview_collections["main"]
        ken_icon = pcoll["Kenimation"]
        ken02_icon = pcoll["Kenimation_02"]
